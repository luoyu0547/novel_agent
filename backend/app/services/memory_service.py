...
class MemoryService:
    ...
    async def _enqueue_sync(self, novel_id: int, user_id: int):
        """Enqueue a retrieval sync job after canonical data changes."""
    ...
    async def create_character(self, user_id: int, novel_id: int, body: CharacterCreate):
        await self.ensure_owned_novel(user_id, novel_id)
        result = await self.character_repo.create(novel_id, body.model_dump())
        await self._enqueue_sync(novel_id, user_id)
        return result
    ...
    async def update_character(self, user_id: int, novel_id: int, character_id: int, body: CharacterUpdate):
        ...
        result = await self.character_repo.update(character, body.model_dump(exclude_unset=True))
        await self._enqueue_sync(novel_id, user_id)
        return result
    ...
    async def create_setting(self, user_id: int, novel_id: int, body: WorldSettingCreate):
        await self.ensure_owned_novel(user_id, novel_id)
        result = await self.setting_repo.create(novel_id, body.model_dump())
        await self._enqueue_sync(novel_id, user_id)
        return result
    ...
    async def update_setting(self, user_id: int, novel_id: int, setting_id: int, body: WorldSettingUpdate):
        ...
        result = await self.setting_repo.update(setting, body.model_dump(exclude_unset=True))
        await self._enqueue_sync(novel_id, user_id)
        return result
