...
class ChapterPlan(Base):
    ...
    volume_arc_id: Mapped[Optional[int]] = mapped_column(ForeignKey("volume_arcs.id"), nullable=True)
    position: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    status: Mapped[str] = mapped_column(default="draft")
    content_json: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
    emotional_effect: Mapped[str] = mapped_column(Text, nullable=False, default="")
    target_word_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    foreshadowing_tasks: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
