...
@pytest.mark.asyncio
async def test_writing_run_input_snapshot_contains_plan_versions(client, novel_and_headers):
    """生成写作运行时 input_snapshot 应包含当前活跃蓝图、卷弧的版本信息。"""
    novel, headers = novel_and_headers
    bp_resp = await client.post(
        f"/api/v1/novels/{novel['id']}/blueprints/generate",
        headers=headers,
    ...
    assert run_resp.status_code == 200
    run = run_resp.json()["data"]
    # input_snapshot 应包含 blueprint_id 和 plan_version_ids
    snapshot = run.get("input_snapshot", {})
    assert isinstance(snapshot, dict)
    # plan_version_ids 不应为空
    assert run.get("plan_version_ids", []) is not None
